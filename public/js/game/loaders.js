...
import { _tank } from './../../assets3d/tank.js';
import { _textures } from './../../assets3d/textures.js';
const _consoleOn = true
const _front = {
	id: new Number(0),
			...
			this.gltfLoader.load(
				...
				(gltf) => {
					...
					if (this.loadCounter === model3dList.length) {
						...
						callbackFunction('_GLTFLoader ok. '+ model3dList.length + ' gltf'+(this.filesCounter>1?'s':'')+' loaded...');
					}
				}, undefined, (error) => {
...
let _TextureLoader = {
	...
	checkEnd: function () {
		if (this.counter === this.filesCounter) {
			this.callbackFunction('_TextureLoader ok. '+ this.filesCounter + ' file'+(this.filesCounter>1?'s':'')+' loaded...')
		}
	},
	addToStack: function (file) {
		this.loadTexture(file, (map) => {
			this.counter++;
			map.name = file.name
			this.textures[file.name] = { map: map, name: file.name, sizes: file.sizes }
